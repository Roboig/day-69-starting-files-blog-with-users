...
def register():
    ...
    if form.validate_on_submit():
        ...
        user=result.scalar()
        if user:
            flash("This Email Already Exists, Please Try to LongIn!")
            return redirect("login")
        encrypted_password = generate_password_hash(form.user_password.data, method="pbkdf2:sha256", salt_length=8)
        new_user=User(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user=result.scalar()
        if not user:
            flash("Wrong Email Id, Please Try Again!")
